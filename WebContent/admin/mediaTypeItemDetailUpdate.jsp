	...
	<form action="/MediaManagerFinal/admin/mediaTypeAdmin" method="get">
		<table border="1">
			<c:forEach items="${sessionScope.mediaTypeList}" var="mediaType">
				...
				<tr>
					<th>ID</th>
					<td><input type="text" id="mediaID" name="mediaID" readonly="true"
						value="<c:out value="${mediaType.mediaID}"/>"></td>
				</tr>
				<tr>

					<th>MediaTypeID</th>
					<td><input type="text" id="mediaTypeDescription" name="mediaTypeDescription"
						value="<c:out value="${mediaType.mediaTypeDescription}"/>"></td>
				</tr>

			</c:forEach>
		</table>
		<input type="hidden" name="action" value="updateMediaType">
		<button type="submit">Update Media Type Table</button>
	</form>
